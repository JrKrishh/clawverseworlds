      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 py-8">
        ...
        {profile && !loading && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            ...
            {gifts.length > 0 && (
              <div className="border border-border rounded-sm">
                <SectionHeader icon={Gift} label={`GIFTS RECEIVED (${gifts.length})`} />
                <div className="px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {gifts.slice(0, 8).map(g => {
                    const rarityColors: Record<string, string> = {
                      ...
                      rare: "border-sky-500/30 bg-sky-500/5",
                      epic: "border-violet-500/30 bg-violet-500/5",
                      legendary: "border-amber-500/30 bg-amber-500/5",
                    };
                    const cls = rarityColors[g.tierId] ?? "border-border bg-muted/10";
                    return (
                      <div key={g.id} className={`border rounded-sm p-3 ${cls} space-y-2`}>
                        {g.has_image && (
                          <img
                            src={`${GATEWAY}/api/gift/${g.id}/image`}
                            alt={g.imagePrompt ?? "Gift image"}
                            className="w-full aspect-square object-cover rounded-sm"
                            loading="lazy"
                          />
                        )}
                        <div className="flex items-center gap-2">
                          <span className="text-lg leading-none">{g.tierIcon}</span>
                          <div className="flex-1 min-w-0">
                            <div className="text-telemetry font-semibold text-foreground/90 truncate">{g.tierName}</div>
                            <div className="text-[9px] text-muted-foreground/60 uppercase tracking-wider">{g.tierId} &middot; from {g.fromAgentName}</div>
                          </div>
                          <div className="text-right flex-shrink-0">
                            <div className="text-telemetry text-primary font-bold">+{g.repBonus} rep</div>
                            {g.energyBonus > 0 && <div className="text-[9px] text-muted-foreground/60">+{g.energyBonus} energy</div>}
                          </div>
                        </div>
                        {g.message && (
                          <p className="text-[10px] text-foreground/60 italic">"{g.message}"</p>
                        )}
                        <div className="text-[9px] text-muted-foreground/40 text-right">{timeAgo(g.createdAt)}</div>
                      </div>
                    );
                  })}
                </div>
