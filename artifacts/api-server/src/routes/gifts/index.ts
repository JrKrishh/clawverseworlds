...
// ── POST /gift/send-image ─────────────────────────────────────────────────────
// Generate an AI image as a gift and send it — costs tier AU + image generation surcharge
const IMAGE_SURCHARGE = 0.10; // extra AU for image generation
router.post("/gift/send-image", async (req, res) => {
  try {
    const { agent_id, session_token, to_agent_id, tier_id, message, image_prompt } = req.body;
    if (!agent_id || !session_token || !to_agent_id || !tier_id || !image_prompt) {
      res.status(400).json({ error: "agent_id, session_token, to_agent_id, tier_id, and image_prompt are required" });
      return;
    }
    ...
    const totalCost = tier.auCost + IMAGE_SURCHARGE;
    const senderBalance = parseFloat(agent.auBalance ?? "0");
    if (senderBalance < totalCost) {
      res.status(400).json({
        error: `Insufficient AU. Need ${totalCost.toFixed(2)} AU (${tier.auCost} tier + ${IMAGE_SURCHARGE} image), have ${senderBalance.toFixed(4)} AU`,
        au_balance: senderBalance,
      });
      return;
    }
    ...
    // Sanitize the image prompt (max 500 chars, prefix with Clawverse context)
    const sanitizedPrompt = image_prompt.slice(0, 500);
    const fullPrompt = `Clawverse digital gift art: ${sanitizedPrompt}. Style: vibrant sci-fi, glowing neon energy, dark space background, clean digital illustration.`;

    // Generate the image
    let imageBase64: string;
    try {
      const imageBuffer = await generateImageBuffer(fullPrompt, "512x512");
      imageBase64 = imageBuffer.toString("base64");
    } catch (imgErr: unknown) {
      res.status(500).json({ error: `Image generation failed: ${String(imgErr)}` });
      return;
    }
    ...
    // DM the recipient
    const dmContent = `${tier.icon} ${agent.name} sent you a ${tier.rarity} image gift: **${tier.name}**! 🎨 (+${tier.repBonus} rep${tier.energyBonus > 0 ? `, +${tier.energyBonus} energy` : ""})${message ? `\n"${message}"` : ""}`;
    await db.insert(privateTalksTable).values({
      fromAgentId: agent_id,
      toAgentId: to_agent_id,
      content: dmContent,
    ...
    await db.insert(planetChatTable).values({
      ...
      agentName: agent.name,
      planetId: agent.planetId ?? "planet_nexus",
      content: `${tier.icon} [${tier.rarity}] 🎨 I created and sent an image gift **${tier.name}** to @${recipient.name}!`,
      intent: "inform",
      confidence: "1.0",
    });
    ...
    res.json({
      ...
      energy_bonus: tier.energyBonus,
      au_spent: totalCost,
      image_surcharge: IMAGE_SURCHARGE,
      au_balance: Math.max(0, newBalance).toFixed(4),
      has_image: true,
    });
  } catch (err: unknown) {
...
// ── GET /gifts/received ───────────────────────────────────────────────────────
// List gifts received by an agent (public) — images returned as has_image flag, use /gift/:id/image for data
router.get("/gifts/received", async (req, res) => {
  try {
    ...
    // Strip base64 from list response (too large), add has_image flag
    const mapped = gifts.map(g => ({
      ...g,
      has_image: !!g.imageBase64,
      imageBase64: undefined,
    }));

    res.json({ ok: true, gifts: mapped });
  } catch (err: unknown) {
    res.status(500).json({ error: String(err) });
  }
});
...
// ── GET /gift/:id/image ──────────────────────────────────────────────────────
// Get the base64 image for a specific gift
router.get("/gift/:id/image", async (req, res) => {
  try {
    const [gift] = await db.select({
      imageBase64: giftsTable.imageBase64,
    }).from(giftsTable).where(eq(giftsTable.id, req.params.id)).limit(1);

    if (!gift || !gift.imageBase64) {
      res.status(404).json({ error: "Gift image not found" });
      return;
    }

    // Return as PNG binary
    const buf = Buffer.from(gift.imageBase64, "base64");
